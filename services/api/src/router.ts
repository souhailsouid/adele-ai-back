...
import {
  ...
  getAccumulationNotifications,
  getGlobalAccumulationNotifications,
...
import { calculateFundDiff, formatDiffMessage } from "./services/fund-diff.service";
    ...
    handler: async (event) => {
      ...
      },
      {
        ...
        handler: async (event) => {
          ...
          try {
            ...
            return await analyzeFundDiffsStrategically(parseInt(id), rawDiffs as any[], noiseThreshold, includeLowConviction);
          } catch (error: any) {
            console.error(`[Strategic Analysis] Error for fund ${id}:`, error);
            throw error;
          }
        },
      },
      ...
      },
      {
        ...
        handler: async (event) => {
          ...
          // Utiliser la version dédupliquée pour éviter le double comptage
          ...
          // Version simple (sans déduplication) pour compatibilité
...

      {
        ...
        handler: async (event) => {
          ...
          // Si only_global=true, retourner l'historique complet des accumulations (tous funds, tous utilisateurs)
          // Sinon, retourner uniquement les notifications de l'utilisateur
          const onlyGlobal = getQueryParam(event, "only_global") === 'true';
          
          if (onlyGlobal) {
            // Options de filtrage par date/trimestre/année
            const options: {
              from_date?: string;
              to_date?: string;
              quarter?: string;
            ...
            return await getGlobalAccumulationNotifications(limit, minQuarters, onlyStrong, Object.keys(options).length > 0 ? options : undefined);
          } else {
      ...
      // ========== Market Pulse - Banner global ==========
      {
        ...
        handler: async (event) => {
          const days = getQueryParam(event, "days") 
            ? parseInt(getQueryParam(event, "days")!) 
            : 30;
          return await getMarketPulse(days);
        },
      },
