 ...
 * Usage: npx ts-node scripts/force-parse-filing.ts <accession_number>
 * Exemple: npx ts-node scripts/force-parse-filing.ts 0000905148-24-003106
 */
...
async function forceParseFiling(accessionNumber: string) {
  ...
  if (holdings && holdings.length > 0) {
    ...
    console.log(`   Voulez-vous quand même forcer le re-parsing ? (y/n)`);
...
if (!accessionNumber) {
  console.error("❌ Usage: npx ts-node scripts/force-parse-filing.ts <accession_number>");
  console.error("   Exemple: npx ts-node scripts/force-parse-filing.ts 0000905148-24-003106");
  process.exit(1);
}
