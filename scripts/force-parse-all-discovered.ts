 ...
 * Usage: npx ts-node scripts/force-parse-all-discovered.ts [--status=DISCOVERED|FAILED|ALL] [--limit=100]
 * Exemple: npx ts-node scripts/force-parse-all-discovered.ts --status=DISCOVERED --limit=50
 */
 ...
 */
async function publishParsingEvent(
  filing: any,
  fundCik: string,
  filingUrl: string
): Promise<boolean> {
...
const options: { status?: "DISCOVERED" | "FAILED" | "ALL"; limit?: number } = {};

for (const arg of args) {
  if (arg.startsWith("--status=")) {
    const status = arg.split("=")[1] as "DISCOVERED" | "FAILED" | "ALL";
